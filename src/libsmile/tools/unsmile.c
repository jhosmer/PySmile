...
static const char unsmile_usage[] = "unsmile <file>";

int main(int argc, char **argv)
{
    int fd, status = 0, ret = 0, idx = 0, utf8 = 1;
    struct stat st;
    const char* path;
    char src[BUFFER_SIZE], dst[BUFFER_SIZE];
    ssize_t bytes_decoded, bytes_read;
    ...
    for (;;) {
        ...
        dbgprintf("read(\"%s\"): %zd bytes", path, bytes_read);
        bytes_decoded = smile_decode_block(dst, BUFFER_SIZE, src, (int)bytes_read);

        // Convert to wide characters
        memset(bytes_out, '\0', BUFFER_SIZE);
        mbstowcs(bytes_out, dst, (size_t)bytes_decoded);
